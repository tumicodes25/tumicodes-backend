...
router.post('/register', authRateLimiter, async (req, res) => {
    try {
        ...
        const emailVerificationToken = jwt.sign(
            { userId: result.insertId, type: 'email_verification' },
            process.env.JWT_SECRET,
            { expiresIn: '24h' }
        );
        ...
        await executeQuery(
            'INSERT INTO notifications (user_id, type, title, message, icon) VALUES (?, ?, ?, ?, ?)',
            [result.insertId, 'info', 'Verify your email', `Please verify your email by visiting: ${verificationLink}`, 'envelope']
        );

        // Send verification email (if configured)
        try {
            const { sendVerificationEmail } = require('../utils/mailer');
            await sendVerificationEmail(email, verificationLink, name || users[0].name || '');
        } catch (mailErr) {
            console.warn('Failed to send verification email:', mailErr.message || mailErr);
        }
...
router.post('/resend-verification', authRateLimiter, async (req, res) => {
    try {
        ...
        await executeQuery(
            'INSERT INTO notifications (user_id, type, title, message, icon) VALUES (?, ?, ?, ?, ?)',
            [user.id, 'info', 'Verify your email', `Please verify your email by visiting: ${verificationLink}`, 'envelope']
        );
        ...
        if (process.env.NODE_ENV === 'development') {
            return res.json({ message: 'Verification link generated', verificationLink });
        }
